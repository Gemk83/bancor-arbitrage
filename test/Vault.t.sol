...
contract VaultTest is Test {
    ...
    function testShouldRevertWhenAttemptingToWithdrawFundsWithoutTheAssetManagerRole() public {
        ...
        vault.withdrawFunds(Token(address(token)), user2, withdrawAmount);
    }
    ...
    function testShouldRevertWhenAttemptingToWithdrawFundsToAnInvalidAddress() public {
        ...
        vault.withdrawFunds(Token(address(token)), payable(address(0)), withdrawAmount);
    }
    ...
    function testAssetManagerShouldBeAbleToWithdrawNativeToken(uint256 withdrawAmount) public {
        ...
        vault.withdrawFunds(TokenLibrary.NATIVE_TOKEN, user2, withdrawAmount);
    ...
    function testWithdrawingFundsShouldEmitEvent() public {
        ...
        vm.prank(user1);
        vm.expectEmit();
        emit FundsWithdrawn(Token(address(token)), user1, user2, withdrawAmount);
        // withdraw token to user2
        vault.withdrawFunds(Token(address(token)), user2, withdrawAmount);
    }
    ...
    function testShouldRevertFundWithdrawalIfReentrancyIsAttempted() public {
        ...
        testReentrancy.tryReenterWithdrawFunds(
            TokenLibrary.NATIVE_TOKEN,
            payable(address(testReentrancy)),
            withdrawAmount
        );
        vm.stopPrank();
    }
